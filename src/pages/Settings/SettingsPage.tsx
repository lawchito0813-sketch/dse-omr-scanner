...
export const SettingsPage = ({ onBack }: SettingsProps) => {
  const { questionCount, answers, setAnswer, setQuestionCount, resetAnswers } = useStore();
  ...
  const stats = useMemo(() => {
    const limitedAnswers = answers.slice(0, questionCount);
        ...
        <div className={styles.settings}>
          ...
          <div className={styles.bulkRow}>
            ...
            <div className={styles.bulkActions}>
              ...
              <Button fullWidth variant="secondary" onClick={resetAnswers}>清空全部</Button>
            </div>
          </div>
              ...
              <div className={styles.optionGroup}>
                {['A', 'B', 'C', 'D'].map((option) => (
                  <button
                    key={option}
                    type="button"
                    className={`${styles.optionButton} ${answers[i] === option ? styles.optionActive : ''}`}
                    onClick={() => {
                      setAnswer(i, option);
